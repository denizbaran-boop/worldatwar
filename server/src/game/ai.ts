  ...
  for (const unit of actorUnits) {
    ...
    for (const tile of snapshot.tiles) {
      ...
      if (role === "reinforcer" && capitalTile) {
        ...
        for (const enemy of enemyUnits) {
          ...
          if (attackDist > 0 && attackDist <= stats.attackRange && canUnitAttackTarget(unit, enemy)) {
            // Hard AI strongly prioritises finishing off weakened enemies
      ...
      if (mode === "Defense" || mode === "Desperation") {
        if (capitalTile) {
          // Hard AI pulls units back more aggressively when capital is under threat
    ...
    if (role !== "scout") {
      ...
      for (const enemy of enemyUnits) {
        ...
        // Hard AI prioritises killing weakened units and capturing valuable targets
